    ...
    <div className="bg-gray-100 dark:bg-gray-900 shadow-lg rounded-xl p-6 md:p-8 mt-6 animate-fade-in">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        ...
        <div className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-md hover:shadow-xl transition-shadow">
          ...
          <p className="mb-2"><strong>Gender:</strong> {data.gender}</p>
          <p className="mb-2"><strong>Delegate Type:</strong> <span className="font-semibold text-accent">{data.delegate_type}</span></p>
        </div>

        {/* Affiliation Info */}
        <div className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-md hover:shadow-xl transition-shadow">
          <h3 className="text-xl font-bold text-secondary border-b-2 border-secondary/20 pb-2 mb-4">🎓 Affiliation</h3>
          <p className="mb-2"><strong>Affiliation:</strong> {data.affiliation}</p>
          <p className="mb-2"><strong>Position:</strong> {data.position}</p>
          <p className="mb-2"><strong>Department:</strong> {data.department}</p>
          <p className="mb-2"><strong>Matric Number:</strong> {data.matric_num}</p>
        </div>
        ...
        <div className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-md hover:shadow-xl transition-shadow md:col-span-2 lg:col-span-1">
          ...
          <p><strong>1st:</strong> {data.committee1} - {data.country1}</p>
          <p><strong>2nd:</strong> {data.committee2} - {data.country2}</p>
          <p><strong>3rd:</strong> {data.committee3} - {data.country3}</p>
        </div>
        ...
        <div className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-md hover:shadow-xl transition-shadow lg:col-span-2">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <p><strong>T-Shirt Size:</strong> {data.tshirt_size.toUpperCase()}</p>
              <p><strong>Experience:</strong> {data.mun_experience}</p>
              <p><strong>Advert Source:</strong> {data.advert}</p>
              <p><strong>Referral Code:</strong> {data.referral || 'N/A'}</p>
              <p className="sm:col-span-2"><strong>Medical:</strong> {data.medical || 'None'}</p>
              <p className="sm:col-span-2"><strong>Dietary:</strong> {data.diet || 'None'}</p>
          </div>
        </div>
